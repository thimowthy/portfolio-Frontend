...
export default function ListUsuarios({ setCreateUser, setListUsers, setUpdateUser, setUser, setDeleteUser, loading, fetchUsers, users }: any) {
    ...
    <>
      <div className="flex min-h-full min-w-full items-center">
        <div className="w-[100%] h-[70vh] mx-auto pt-10 px-7 bg-[#fff] rounded-lg flex flex-col overflow-y-auto relative">
          ...
          <div className="flex gap-3 bg-lime-200 box-border w-[250px] rounded-lg py-3 px-3 absolute right-8 top-7 cursor-pointer" onClick={() => handleAddUser()}>
            <Image src={AddUser} alt="Adicionar Usuário" />
            Cadastrar novo usuário
          </div>
          {!users.length && !loading ? <p className="text-center pt-16">Nenhum usuário cadastrado.</p> : 
            <>
